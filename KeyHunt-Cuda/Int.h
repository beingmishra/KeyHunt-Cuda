...
public:
	...
	{
		// Convert the Int object to a string and append it to the provided stream
		stream << this->bits[0];
		for (int i = 1; i < NUM_UINT64; i++)
		{
			stream << std::setw(16) << std::setfill('0') << std::hex << this->bits[i];
		}
	}
	void ToString(char *buffer, size_t bufferSize)
	{
		// Convert the Int object to a C-style string and store it in the provided buffer
		...
		{
			// Buffer is not large enough to hold the entire string
			buffer[0] = '\0';
		}
	}
...
private:
	...
	static const int NUM_UINT64 = 4; // Assuming the Int class uses 4 uint64_t values
	uint64_t bits[NUM_UINT64];
};
